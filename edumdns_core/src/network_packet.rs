...
impl<'a> DataLinkPacket<'a> {
    pub fn from_slice(slice: &'a mut [u8]) -> Result<DataLinkPacket<'a>, CoreError> {
        let new_packet = MutableEthernetPacket::new(slice).ok_or(CoreError::new(
    ...
    pub fn from_buffer(
        ...
        let mut new_packet = MutableEthernetPacket::new(&mut value[..]).ok_or(CoreError::new(
            CoreErrorKind::PacketConstructionError,
            "Could not construct an EthernetPacket",
        ))?;
    ...
    pub fn get_ip_metadata(&self) -> Result<IpMetadata, CoreError> {
        let metadata = match self {
            IpPacket::Ipv4Packet(p) => IpMetadata {
                src_ip: IpNetwork(ipnetwork::IpNetwork::V4(ipnetwork::Ipv4Network::new(
                    p.get_source(),
                    32,
                )?)),
                dst_ip: IpNetwork(ipnetwork::IpNetwork::V4(ipnetwork::Ipv4Network::new(
                    p.get_destination(),
                    32,
                )?)),
            },
            IpPacket::Ipv6Packet(p) => IpMetadata {
                src_ip: IpNetwork(ipnetwork::IpNetwork::V6(ipnetwork::Ipv6Network::new(
                    p.get_source(),
                    128,
                )?)),
                dst_ip: IpNetwork(ipnetwork::IpNetwork::V6(ipnetwork::Ipv6Network::new(
                    p.get_destination(),
                    128,
                )?)),
            },
