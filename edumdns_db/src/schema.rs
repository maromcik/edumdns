...
diesel::table! {
    ...
    user_probe_permission (user_id, probe_id) {
        user_id -> Int8,
        probe_id -> Uuid,
        permission -> Permission,
    }
...
diesel::joinable!(user_probe_permission -> probe (probe_id));
diesel::joinable!(user_probe_permission -> user (user_id));
