...
use edumdns_core::app_packet::{AppPacket, LocalAppPacket, LocalCommandPacket, PacketTransmitRequestPacket};
    ...
    for (device, request) in requests {
        ...
        if !request.permanent {
            ...
            tokio::spawn(async move {
                ...
                if let Err(e) = device_repo_local
                    .delete_packet_transmit_request(&id)
                    .await
                {
                    warn!(
                ...
                tx_local
                    .send(AppPacket::Local(LocalAppPacket::Command(
                        LocalCommandPacket::StopTransmitDevicePackets(packet_transmit_request_local),
                    )))
                    .await
            });
        }
